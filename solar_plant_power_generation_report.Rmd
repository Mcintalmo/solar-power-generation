...
generation %>%
  ...
  unnest() %>%
  kbl(caption = "Power Generation Source Keys Separated by Plant ID") %>%
  kable_material()
```
...
weather %>%
  ...
  pivot_wider(names_from = "plant_id", values_from = "source_key") %>%
  unnest() %>%
  kbl(caption = "Weather Data Source Keys Separated by Plant ID") %>%
  kable_material()
```
...
we expect to see 143616 observations. 

The remaining 7140 observations are missing, and will be considered NA for time series analysis.
