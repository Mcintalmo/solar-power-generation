...
kableExtra and bookdown were used only in the report to generate more visually 
...
```{r sensor-anamolies, fig.cap = "Sensor Aanomalies Over Time"}
solar %>%
  na.omit() %>%
...
Figure \@ref(fig:sensor-anamolies) displays anomalies in the sensor data.
...
The result is predictably perdiodic, peaking at about 1:15 PM each day.
...
The relationship between the DC Power and AC Power is shown in table 
\@ref(tab:ac-dc-correlation).
...
correlated. 
A linear regression method was used to predict dc output based purely on 
irradiation, using the train set to predict against the test set.
The result is shown in tabe \@ref{tab:irradiation-effect}
...
We can get a better understanding of how temperature effects the dc output by
...
less than 0 to 0.
The results of clamping may be seen in tab \@ref(tab:clamped)
...
shows the final fit.
The colored lins represent the observed DC output at teach plant.
...
arima_source_rmse <- RMSE(predicted_power$pred, test_set$dc_power)
...
result %>%
  ...
  kable_material()
```
The large RMSE, 558, is even worse than the average.
Interestingly, this suggests that the data was wildly over fitted when only going
by source.
The inidiviudalized-by-source fits are showsn in figure \@ref{fig:arima-pred}.

The inherent smoothing of taking the average dc power improved the fit.
When the averaging was removed, the fit deteriorated.
...
predicted_power %>%
  group_by(generation_source) %>%
  ggplot(aes(x = date_time)) +
  geom_point(aes(y = dc_power), alpha = 0.75) +
  geom_line(aes(y = pred, color = generation_source), size = 1) +
  facet_grid(generation_source ~ .) + 
  theme(legend.position = "none") +
  labs(x = "Time",
       y = "DC Power")
```
...
Though their previously recorded values could be incorperated with lag, AC power,
...
Interestingly, ambient temperature remains a relavent predictor.
And despite the early assumption that the time of day help little importace,
it still makes an appearance on.

Day, minute, and generation source are all of negligable effect.
...
```{r rf-plot, fig.cap = "Random Forest Prediction by Generation Source", fig.height = 28}
...
## Future Work
Linear Model by generation source
Incorporating more predictable weather data
Predicting based on purely predictable weather data (no module temp, only ambient temp and irradiation)
Predicting when a solar panel is failing
Ensemble modelling
